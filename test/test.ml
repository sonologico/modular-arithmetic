...
let rnd () = Random.int 100000

let op _ =
  for i = 1 to n do
    let module M = Mod_arith.Make (struct
      let modulus = Random.int 100 + 1
    end) in
    let a, b = (rnd (), rnd ()) in
    ...
    with Mod_arith.No_mul_inverse (_, _) ->
      let open M in
      assert_raises
        (Mod_arith.No_mul_inverse (to_int mb, modulus))
        (fun () -> ma / mb)
  done

let tests = "Tests" >::: ["Op" >:: op]

let () = Random.self_init () ; run_test_tt_main tests
